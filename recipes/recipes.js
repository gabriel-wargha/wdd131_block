...
function RecipeTemplate(recipes) {
	...
	<p>${generateTags(recipes.tags)}</p>
        ...
        <span class= "rating" role="img" aria-label="Rating: ${
                    ...
                    ${generateRating(recipes.rating)}
                </span>
                <p class="paragraph hide">${recipes.description}</p>
				</div>
					</div>`;
}
...
function init() {
	const random_recipe = Random_recipe();
	const template = RecipeTemplate(random_recipe);
...
function filter(query) {
	...
	const filtered = recipes.filter((recipes) => {
		return (
			...
			recipes.tags.some((tag) => tag.toLowerCase().includes(query)) ||
			recipes.recipeIngredient.some((ingredient) =>
...
function renderRecipes(recipes) {
	...
	recipesContainer.innerHTML = recipes.map(RecipeTemplate).join('');
}
